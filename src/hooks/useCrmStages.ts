...
// PRODUCTION webhook for the "TT CRM Stages - Quoting Opp + Leads" workflow:
...
export interface QuotingOpp {
  count: number;
  value: number;
  leads: QuotingOppLead[];
}
export interface CrmStages {
  quotingOpp: QuotingOpp | null;
  totalLeads: number;
}
    ...
    try {
      ...
      if (qo && typeof qo.count === "number") {
        const leads: QuotingOppLead[] = Array.isArray(qo.leads)
          ...
          : [];
        setData({ quotingOpp: { count: qo.count, value: Number(qo.value) || 0, leads }, totalLeads });
      } else {
        setData({ quotingOpp: null, totalLeads });
      }
    } catch {
      /* swallow — card shows the count placeholder */
    }
  }, []);
