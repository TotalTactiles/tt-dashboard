...
interface DebtRegisterRowProps {
  ...
  computedRepaymentsRemaining: number;
  flat: boolean;
  isDragOver: boolean;
  autoEdit: boolean;
...
const DebtRegisterRow = memo(({
  facility, computedBalance, computedRepaid, computedInterest, computedRepaymentsRemaining, flat, isDragOver, autoEdit,
  ...
  return (
    ...
    >
      ...
      <td className="py-1.5 px-2 text-right whitespace-nowrap text-rose-300/90" title="Total future cash: Monthly × payments left (matches accountant)">
        {fmtCurrency(computedRepaymentsRemaining)}
      </td>
      <td className="py-1.5 px-2 text-right whitespace-nowrap">
        {isEditing ? (
              ...
              <tbody className="font-mono">
                {debts.map((d, index) => {
                  ...
                  <DebtRegisterRow
                    ...
                    computedRepaymentsRemaining={(c as any)?.repaymentsRemaining ?? 0}
                    flat={!!c?._flat}
                ...
                {debts.length === 0 && (
                  <tr><td colSpan={14} className="py-6 text-center text-muted-foreground whitespace-nowrap">No facilities. Click "Add Facility" to start.</td></tr>
                )}
              </tbody>
              {debts.length > 0 && (
                <tfoot>
                  <tr className="border-t border-border font-mono text-foreground bg-muted/10">
                    ...
                    <td className="py-2 px-2 text-right font-semibold whitespace-nowrap text-rose-300/90">{fmtCurrency(totals.totalRepaymentsRemaining)}</td>
...
const ChartsSection = ({
  ...
  const [selectedFacilityKey, setSelectedFacilityKey] = useState<FacilityKey>("custom");
  ...
  const debtStripped = useMemo(() => {
    ...
    // Forward months receive a flat 70% haircut (debt-stripped operating cash flow).
    ...
    const _forwardContracted = (io ?? [])
      ...
      .map((d: any) => {
        ...
        // Legacy flat 70% haircut on all forward months (debt-stripped).
    ...
    if (serviceabilityView === "actuals") {
      ...
      const _layer = serviceabilityView === "with_ylw"
        ? "signed + verbal pipeline"
        : "signed contracts";
      avg6Eq = `3 recent actual (avg ${_f(_actAvg)}) + 3 forward ${_layer} @70% haircut (avg ${_f(_fwdAvg)}) → blended ÷ 6 = ${_f(avg6MonthBlended)}`;
    }
        ...
        <div className="flex items-center gap-2 mb-4 flex-wrap">
          ...
          <div className="inline-flex bg-white/5 rounded-xl p-1 border border-white/10">
            {[
              ...
              { key: "with_grn", label: "✅ With GRNs", desc: "Actuals + signed contracts at 70%" },
              { key: "with_ylw", label: "🤝 With GRNs & YLWs", desc: "Actuals + signed + verbal pipeline at 70%" },
            ].map(opt => (
              <button
                    ...
                    return (
                      <div className="bg-white/[0.03] border border-white/10 rounded-xl p-4">
                        ...
                        <p className="text-[9px] text-muted-foreground italic mt-0.5">
                          ...
                          {serviceabilityView === "with_ylw" && "6m blended: actuals + signed + verbal pipeline at 70%"}
                        </p>
                              ...
                              <div>
                                ...
                                <div className="space-y-1.5 text-[11px] font-mono">
                                  ...
                                  <p className="text-[9px] text-muted-foreground/70 italic pl-2">
                                    ...
                                    {serviceabilityView === "with_grn" && "Actuals + signed contracts at 70%."}
                                    {serviceabilityView === "with_ylw" && "Actuals + signed + verbal pipeline at 70%."}
                                  </p>
