import { useState, useEffect, useMemo } from "react";
import { Dialog, DialogContent, DialogHeader, DialogTitle } from "@/components/ui/dialog";
import { Input } from "@/components/ui/input";
import { Textarea } from "@/components/ui/textarea";
import { Switch } from "@/components/ui/switch";
import { Label } from "@/components/ui/label";
import { type LiveCalendarEvent, type ZohoProject } from "@/contexts/DashboardDataContext";
import { Loader2, AlignLeft, MapPin, Calendar as CalendarIcon, Clock, Users, Plus } from "lucide-react";

interface EventModalProps {
  open: boolean;
  onClose: () => void;
  event: LiveCalendarEvent | null;
  onSave: (action: "create" | "update" | "delete", data: Partial<LiveCalendarEvent>) => Promise<void>;
  selectedDate: Date;
  zohoProjects?: ZohoProject[];
}

const EVENT_TYPES = ["Meeting", "Deadline", "Milestone", "Care", "Valuation", "Distribution"];

const TYPE_COLORS: Record<string, string> = {
  Meeting: "#378ADD",
  Deadline: "#E24B4A",
  Milestone: "#7F77DD",
  Care: "#639922",
  Valuation: "#BA7517",
  Distribution: "#1D9E75",
};

const pad = (n: number) => n.toString().padStart(2, "0");
const toDateInput = (d: Date) => `${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}`;
const toTimeInput = (d: Date) => `${pad(d.getHours())}:${pad(d.getMinutes())}`;

const initialsFor = (raw: string) => {
  const s = raw.trim();
  if (!s) return "?";
  const name = s.includes("@") ? s.split("@")[0] : s;
  const parts = name.replace(/[._-]+/g, " ").split(/\s+/).filter(Boolean);
  const letters = (parts[0]?.[0] ?? "") + (parts[1]?.[0] ?? "");
  return (letters || name.slice(0, 2)).toUpperCase();
};

const avatarBg = (raw: string) => {
  let h = 0;
  for (let i = 0; i < raw.length; i++) h = (h * 31 + raw.charCodeAt(i)) % 360;
  return `hsl(${h} 55% 42%)`;
};

const EventModal = ({ open, onClose, event, onSave, selectedDate, zohoProjects = [] }: EventModalProps) => {
  console.log('[EventModal] zohoProjects:', zohoProjects);
  const isEditing = !!event;

  const [title, setTitle] = useState("");
  const [date, setDate] = useState("");
  const [startTime, setStartTime] = useState("09:00");
  const [endTime, setEndTime] = useState("10:00");
  const [allDay, setAllDay] = useState(false);
  const [type, setType] = useState("Meeting");
  const [description, setDescription] = useState("");
  const [location, setLocation] = useState("");
  const [attendeesStr, setAttendeesStr] = useState("");
  const [attendeeDraft, setAttendeeDraft] = useState("");
  const [showAttendeeInput, setShowAttendeeInput] = useState(false);
  const [loading, setLoading] = useState(false);
  const [errors, setErrors] = useState<{ title?: boolean; date?: boolean; project?: boolean }>({});
  const [confirmDelete, setConfirmDelete] = useState(false);

  // Create-mode: Google Calendar vs Zoho Projects
  const [createSource, setCreateSource] = useState<"Google Calendar" | "Zoho Projects">("Google Calendar");
  const [zohoProjectId, setZohoProjectId] = useState<string>("");
  const [zohoMode, setZohoMode] = useState<"task" | "subtask">("task");
  const [zohoParentTaskId, setZohoParentTaskId] = useState<string>("");

  useEffect(() => {
    if (!open) {
      setLoading(false);
      setErrors({});
      setConfirmDelete(false);
      setShowAttendeeInput(false);
      setAttendeeDraft("");
      return;
    }
    if (event) {
      setTitle(event.title);
      const s = new Date(event.start);
      const e = new Date(event.end);
      setDate(toDateInput(s));
      setStartTime(toTimeInput(s));
      setEndTime(toTimeInput(e));
      setAllDay(event.allDay);
      setType(event.type);
      setDescription(event.description || "");
      setLocation(event.location || "");
      setAttendeesStr((event.attendees || []).join(", "));
    } else {
      setTitle("");
      setDate(toDateInput(selectedDate));
      setStartTime("09:00");
      setEndTime("10:00");
      setAllDay(false);
      setType("Meeting");
      setDescription("");
      setLocation("");
      setAttendeesStr("");
      setCreateSource("Google Calendar");
      setZohoProjectId("");
      setZohoMode("task");
      setZohoParentTaskId("");
    }
  }, [event, selectedDate, open]);

  const attendeesList = useMemo(
    () => attendeesStr.split(",").map((a) => a.trim()).filter(Boolean),
    [attendeesStr]
  );

  const addAttendee = () => {
    const v = attendeeDraft.trim();
    if (!v) { setShowAttendeeInput(false); return; }
    const next = [...attendeesList, v];
    setAttendeesStr(next.join(", "));
    setAttendeeDraft("");
    setShowAttendeeInput(false);
  };

  const removeAttendee = (idx: number) => {
    const next = attendeesList.filter((_, i) => i !== idx);
    setAttendeesStr(next.join(", "));
  };

  const handleSave = async () => {
    const isZohoCreate = !isEditing && createSource === "Zoho Projects";
    const newErrors: { title?: boolean; date?: boolean; project?: boolean } = {};
    if (!title.trim()) newErrors.title = true;
    if (!date) newErrors.date = true;
    if (isZohoCreate && !zohoProjectId) newErrors.project = true;
    if (Object.keys(newErrors).length > 0) {
      setErrors(newErrors);
      return;
    }
    setErrors({});

    const effectiveAllDay = isZohoCreate ? true : allDay;
    const startISO = effectiveAllDay
      ? new Date(`${date}T00:00:00`).toISOString()
      : new Date(`${date}T${startTime}:00`).toISOString();
    const endISO = effectiveAllDay
      ? new Date(`${date}T23:59:59`).toISOString()
      : new Date(`${date}T${endTime}:00`).toISOString();

    const attendees = attendeesList;

    setLoading(true);
    if (isZohoCreate) {
      await onSave("create", {
        title: title.trim(),
        description: description.trim() || "",
        start: startISO,
        end: endISO,
        startDate: date,
        endDate: date,
        allDay: true,
        type: "Task",
        source: "Zoho Projects",
        projectId: zohoProjectId,
        parentTaskId: zohoMode === "subtask" ? zohoParentTaskId : "",
      } as any);
    } else {
      await onSave(isEditing ? "update" : "create", {
        title: title.trim(),
        description: description.trim() || "",
        location: location.trim() || "",
        start: startISO,
        end: endISO,
        startDate: date,
        endDate: date,
        allDay,
        type,
        attendees,
        googleId: event?.googleId,
        zohoId: event?.zohoId,
        source: event?.source ?? "Google Calendar",
        projectId: (event as any)?.projectId,
      } as any);
    }
    setLoading(false);
  };

  const handleDelete = async () => {
    if (!confirmDelete) {
      setConfirmDelete(true);
      return;
    }
    setLoading(true);
    await onSave("delete", {
      googleId: event?.googleId,
      zohoId: event?.zohoId,
      source: event?.source,
      projectId: (event as any)?.projectId,
      parentTaskId: (event as any)?.parentTaskId ?? "",
    } as any);
    setLoading(false);
  };

  const isZohoCreate = !isEditing && createSource === "Zoho Projects";
  const isZohoEdit = isEditing && event?.source === "Zoho Projects";
  const selectedProject = zohoProjects.find((p) => p.id === zohoProjectId);
  const primaryLabel = isEditing ? "Save" : "Add Event";

  return (
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="w-[calc(100vw-2rem)] sm:max-w-[440px] max-h-[85vh] overflow-y-auto p-6 rounded-2xl shadow-2xl">
        <DialogHeader className="sr-only">
          <DialogTitle>{isEditing ? "Edit Event" : "Add Event"}</DialogTitle>
        </DialogHeader>

        <div className="flex flex-col gap-5 min-w-0">
          {/* Source segmented control (create mode only) */}
          {!isEditing && (
            <div className="flex items-center gap-1 p-1 bg-secondary/50 rounded-lg">
              {(["Google Calendar", "Zoho Projects"] as const).map((src) => (
                <button
                  key={src}
                  onClick={() => setCreateSource(src)}
                  className={[
                    "flex-1 px-3 py-1.5 rounded-md text-xs font-medium transition-colors",
                    createSource === src
                      ? "bg-background text-foreground shadow-sm"
                      : "text-muted-foreground hover:text-foreground",
                  ].join(" ")}
                >
                  {src}
                </button>
              ))}
            </div>
          )}

          {/* Zoho project + task/subtask selectors */}
          {isZohoCreate && (
            <div className="flex flex-col gap-2 min-w-0">
              <div>
                <Label className="text-[11px] font-normal text-muted-foreground mb-1 block">Project</Label>
                <select
                  value={zohoProjectId}
                  onChange={(e) => { setZohoProjectId(e.target.value); setZohoParentTaskId(""); setErrors((p) => ({ ...p, project: false })); }}
                  className={[
                    "w-full h-9 text-sm rounded-lg bg-secondary/50 border-0 px-2.5",
                    errors.project ? "ring-1 ring-destructive" : "",
                  ].join(" ")}
                >
                  <option value="">Select project…</option>
                  {zohoProjects.map((p) => (
                    <option key={p.id} value={p.id}>{p.name}</option>
                  ))}
                </select>
                {errors.project && <p className="text-[11px] text-destructive mt-1">Project is required</p>}
                {zohoProjects.length === 0 && (
                  <p className="text-[11px] text-muted-foreground mt-1">No Zoho projects loaded yet — try syncing.</p>
                )}
              </div>

              <div className="flex items-center gap-1 p-1 bg-secondary/50 rounded-lg">
                {(["task", "subtask"] as const).map((m) => (
                  <button
                    key={m}
                    onClick={() => setZohoMode(m)}
                    className={[
                      "flex-1 px-3 py-1 rounded-md text-[11px] font-medium transition-colors",
                      zohoMode === m
                        ? "bg-background text-foreground shadow-sm"
                        : "text-muted-foreground hover:text-foreground",
                    ].join(" ")}
                  >
                    {m === "task" ? "New task" : "Subtask of…"}
                  </button>
                ))}
              </div>

              {zohoMode === "subtask" && (
                <select
                  value={zohoParentTaskId}
                  onChange={(e) => setZohoParentTaskId(e.target.value)}
                  disabled={!selectedProject}
                  className="w-full h-9 text-sm rounded-lg bg-secondary/50 border-0 px-2.5 disabled:opacity-50"
                >
                  <option value="">Select parent task…</option>
                  {selectedProject?.tasks.map((t) => (
                    <option key={t.id} value={t.id}>{t.name}</option>
                  ))}
                </select>
              )}
            </div>
          )}

          {/* Title — large, borderless */}
          <div className="min-w-0">
            <Input
              value={title}
              onChange={(e) => { setTitle(e.target.value); setErrors((p) => ({ ...p, title: false })); }}
              placeholder="Add title"
              className={[
                "w-full min-w-0 text-xl font-semibold tracking-tight border-0 border-b rounded-none bg-transparent px-0 py-2 h-auto",
                "placeholder:text-muted-foreground/60 placeholder:font-normal",
                "focus-visible:ring-0 focus-visible:border-b-ring",
                errors.title ? "border-b-destructive focus-visible:border-b-destructive" : "border-b-border/40",
              ].join(" ")}
            />
            {errors.title && <p className="text-[11px] text-destructive mt-1">Title is required</p>}
          </div>

          {/* Date row */}
          <div className="flex items-center gap-3 min-w-0">
            <CalendarIcon className="h-4 w-4 text-muted-foreground shrink-0" />
            <div className="flex-1 min-w-0">
              <Input
                type="date"
                value={date}
                onChange={(e) => { setDate(e.target.value); setErrors((p) => ({ ...p, date: false })); }}
                className={[
                  "w-full min-w-0 h-9 text-sm border-0 bg-secondary/50 rounded-lg px-2.5",
                  errors.date ? "ring-1 ring-destructive" : "",
                ].join(" ")}
              />
              {errors.date && <p className="text-[11px] text-destructive mt-1">Date is required</p>}
            </div>
          </div>

          {/* Time row + All-day (hidden for Zoho create) */}
          {!isZohoCreate && (
            <div className="flex items-center gap-3 min-w-0">
              <Clock className="h-4 w-4 text-muted-foreground shrink-0" />
              <div className="flex-1 min-w-0 flex items-center gap-2">
                {!allDay ? (
                  <>
                    <Input
                      type="time"
                      value={startTime}
                      onChange={(e) => setStartTime(e.target.value)}
                      className="flex-1 basis-0 min-w-0 h-9 text-sm border-0 bg-secondary/50 rounded-lg px-2.5"
                    />
                    <span className="text-muted-foreground shrink-0">–</span>
                    <Input
                      type="time"
                      value={endTime}
                      onChange={(e) => setEndTime(e.target.value)}
                      className="flex-1 basis-0 min-w-0 h-9 text-sm border-0 bg-secondary/50 rounded-lg px-2.5"
                    />
                  </>
                ) : (
                  <span className="flex-1 text-sm text-muted-foreground">All-day</span>
                )}
                <div className="flex items-center gap-1.5 shrink-0 pl-1">
                  <Switch
                    id="allDay"
                    checked={allDay}
                    onCheckedChange={setAllDay}
                    className="scale-90"
                  />
                  <Label htmlFor="allDay" className="text-[11px] text-muted-foreground whitespace-nowrap cursor-pointer">
                    All-day
                  </Label>
                </div>
              </div>
            </div>
          )}

          {/* Location row (hidden for Zoho create) */}
          {!isZohoCreate && (
            <div className="flex items-center gap-3 min-w-0">
              <MapPin className="h-4 w-4 text-muted-foreground shrink-0" />
              <Input
                value={location}
                onChange={(e) => setLocation(e.target.value)}
                placeholder="Add location"
                className="flex-1 min-w-0 h-9 text-sm border-0 bg-secondary/50 rounded-lg px-2.5 placeholder:text-muted-foreground/60"
              />
            </div>
          )}

          {/* Type chips (hidden for Zoho create) */}
          {!isZohoCreate && (
            <div className="flex flex-wrap gap-1.5 min-w-0">
              {EVENT_TYPES.map((t) => {
                const active = type === t;
                return (
                  <button
                    key={t}
                    onClick={() => setType(t)}
                    className="px-2.5 py-1 rounded-full text-[11px] font-medium transition-all duration-150"
                    style={
                      active
                        ? { backgroundColor: TYPE_COLORS[t], color: "#fff", boxShadow: `0 0 0 2px ${TYPE_COLORS[t]}22` }
                        : { backgroundColor: `${TYPE_COLORS[t]}14`, border: `1px solid ${TYPE_COLORS[t]}55`, color: TYPE_COLORS[t] }
                    }
                  >
                    {t}
                  </button>
                );
              })}
            </div>
          )}

          {/* Description */}
          <div className="flex items-start gap-3 min-w-0">
            <AlignLeft className="h-4 w-4 text-muted-foreground mt-2 shrink-0" />
            <Textarea
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              placeholder="Add description"
              rows={2}
              className="flex-1 min-w-0 min-h-[56px] text-sm border-0 bg-secondary/50 rounded-lg px-2.5 py-2 resize-y placeholder:text-muted-foreground/60 focus-visible:ring-0"
            />
          </div>

          {/* Attendees as avatar row (hidden for Zoho create) */}
          {!isZohoCreate && (
            <div className="flex items-start gap-3 min-w-0">
              <Users className="h-4 w-4 text-muted-foreground mt-1.5 shrink-0" />
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-1.5">
                  {attendeesList.map((a, i) => (
                    <button
                      key={`${a}-${i}`}
                      type="button"
                      onClick={() => removeAttendee(i)}
                      title={`${a} — click to remove`}
                      className="h-7 w-7 rounded-full text-[10px] font-semibold text-white flex items-center justify-center ring-2 ring-background hover:opacity-80 transition-opacity"
                      style={{ backgroundColor: avatarBg(a) }}
                    >
                      {initialsFor(a)}
                    </button>
                  ))}
                  {showAttendeeInput ? (
                    <Input
                      autoFocus
                      value={attendeeDraft}
                      onChange={(e) => setAttendeeDraft(e.target.value)}
                      onBlur={addAttendee}
                      onKeyDown={(e) => {
                        if (e.key === "Enter") { e.preventDefault(); addAttendee(); }
                        if (e.key === "Escape") { setAttendeeDraft(""); setShowAttendeeInput(false); }
                      }}
                      placeholder="email@example.com"
                      className="h-7 w-56 max-w-full text-xs border-0 bg-secondary/50 rounded-full px-3"
                    />
                  ) : (
                    <button
                      type="button"
                      onClick={() => setShowAttendeeInput(true)}
                      className="h-7 w-7 rounded-full bg-secondary/60 hover:bg-secondary text-muted-foreground flex items-center justify-center transition-colors"
                      aria-label="Add attendee"
                    >
                      <Plus className="h-3.5 w-3.5" />
                    </button>
                  )}
                </div>
                {attendeesList.length === 0 && !showAttendeeInput && (
                  <p className="text-[11px] text-muted-foreground mt-1">No attendees yet</p>
                )}
              </div>
            </div>
          )}

          {/* Actions */}
          <div className="flex flex-wrap items-center justify-between gap-2 pt-1">
            <div className="min-w-0 flex flex-col gap-1">
              {isEditing && (
                <button
                  onClick={handleDelete}
                  disabled={loading}
                  className="text-xs text-destructive hover:text-destructive/80 font-medium transition-colors disabled:opacity-50 text-left"
                >
                  {confirmDelete ? "Confirm Delete" : "Delete"}
                </button>
              )}
              {isZohoEdit && (
                <span className="text-[11px] text-muted-foreground">
                  Managed in Zoho Projects — edits sync back.
                </span>
              )}
            </div>
            <div className="flex items-center gap-2 flex-wrap">
              <button
                onClick={onClose}
                disabled={loading}
                className="px-4 py-2 rounded-lg text-sm font-medium text-muted-foreground hover:bg-secondary transition-colors disabled:opacity-50"
              >
                Cancel
              </button>
              <button
                onClick={handleSave}
                disabled={loading}
                className="px-5 py-2 rounded-lg bg-primary text-primary-foreground text-sm font-semibold hover:bg-primary/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed inline-flex items-center justify-center gap-2 shadow-sm"
              >
                {loading && <Loader2 className="h-4 w-4 animate-spin" />}
                {loading ? "Saving…" : primaryLabel}
              </button>
            </div>
          </div>
          {confirmDelete && (
            <p className="text-[11px] text-destructive text-center -mt-2">
              {event?.source === "Zoho Projects"
                ? "Delete this task from Zoho Projects? Click again to confirm."
                : "Delete this event from Google Calendar? Click again to confirm."}
            </p>
          )}
        </div>
      </DialogContent>
    </Dialog>
  );
};

export default EventModal;
